...
function ExampleLogViewer(props : any){
  ...
  const accid = entity?.metadata?.annotations?.['harness.io/cicd-accountIdentifier'];
  ...
  }, [accid, backendBaseUrl, dataSource, logKeys, logToken]);
  ...
  const startStream = React.useCallback(
    ...
    [accid, backendBaseUrl, closeStream, dataSource, logKeys, logToken]
  );
  ...
  }, [dataSource, logToken, startStream])
  ...
  return (
    <div style={{ height: '35vh', width: '100%' }}>
      <LogViewer text={dataSource === 'blob' ? finalLog.join('\n') : logStream.join('\n')}/>
    </div>
  )
};
